...
locals {
  ...
  # Use first 2 AZs for a realistic per-AZ NAT setup
...
############################################
# Private subnets + private route tables (one per AZ)
############################################
...
resource "aws_subnet" "private" {
  ...
  # Use a different subnet range from public subnets
...
############################################
# NAT Gateway module call (per AZ)
############################################
...
module "nat_gateway" {
  ...
  public_subnet_ids_by_az = { for az, s in aws_subnet.public : az => s.id }

  private_route_table_ids_by_az = { for az, rt in aws_route_table.private : az => rt.id }
